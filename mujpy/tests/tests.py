...
# -------------------------------------------------------------
def execute_model_evaluation(test_dict):
    """
    ...
    fit_type = test_dict["fit_type"]
    json = test_dict["json"]
    datafile = resources.files("mujpy.tests").joinpath(self.data).joinpath('deltat_tdc_gps_0822.bin')
    fitdir = resources.files("mujpy.tests").joinpath('fit_gps').joinpath(model+json)
    offset = "20"
...
if __name__ == "__main__":
    ...
    for index, case in enumerate(test_matrix, start=1):
        runner.run_test_case(
            index=index,
            test_dict = case,
            test_logic_func=execute_model_evaluation
        )
